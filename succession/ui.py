...
def render_prompts_sidebar():
    scfg.ensure_session()
    prompts = scfg.get_prompts()
    rubrics = scfg.get_rubrics()

    with st.expander("🧩 プロンプト設定（サクセッション）", expanded=False):
...
def render_assessment_ui():
    ...
    # Sidebar: back, API, dry-run, prompt settings
    with st.sidebar:
        st.header("📋 メニュー")
    ...
    if st.button("AI評価を実行する", type="primary"):
        df = pd.DataFrame({"項目": ["全入力"], "あなたの考え": [user_bulk_text.strip()]})
